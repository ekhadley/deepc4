...
torch.set_printoptions(threshold=100, sci_mode=False, linewidth=200, precision=4)
...
def playAgentMatch(learner:vpoAgent, opponent:vpoAgent, boardShape, numGames, show=False):
    ...
    for turn in range(maxGameLen-1):
        ...
        dists, acts = agent.chooseAction(boards)
        if learnersMove:
...
def train(saveDir,
    ...
    beatBest, matchWR = False, 0
    for match in (t:=trange(numMatches, ncols=140, unit="matches")):
        desc = blue + bold
        ...
        if match > 0 and match%testEvery == 0: # play test match to see if learner has surpassed prev best policy
            ...
            learner.train()
            matchWR = (torch.sum(m_vals[:testMatchSize]).item()/(testMatchSize) + 1)/2
            beatBest = matchWR >= wrThresh
            #print(bold, pink, [e.item() for e in m_vals], m_vals.shape, torch.sum(m_vals).item(), matchWR, endc)
            if beatBest:
                opponents.append(learner.policyStateDict())
                learner.save(saveDir, f"{match}")
        ...
        wandb.log({"gameLength": numTurns//matchSize, "loss":loss, "matchWR":matchWR, "numAgents":len(opponents), "score":torch.sum(val).item(), "pred_acc":pred_acc})
